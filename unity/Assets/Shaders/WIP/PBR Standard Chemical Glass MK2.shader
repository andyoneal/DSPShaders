Shader "VF Shaders/Forward/PBR Standard Chemical Glass MK2 REPLACE" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            v2f vert(appdata_full v, uint vertexID : SV_VertexID, uint instanceID : SV_InstanceID) {
                ...
                r6.xyz = unity_ObjectToWorld[1].yzx * worldTangent.xxx;
                r0.xyz = unity_ObjectToWorld[0].yzx * worldTangent.yyy + r6.xyz;
                r0.xyz = unity_ObjectToWorld[2].yzx * worldTangent.zzz + r0.xyz;
                worldTangent = normalize(r0.xyz); //r0.xyz

                r6.xyz = worldNormal2.yzx * worldTangent.yzx - worldNormal2.zxy * worldTangent.xyz;
                float3 worldBinormal = r6.xyz * unity_WorldTransformParams.w * v.tangent.w; //r6.xyz
            ...
            fout frag(v2f i) {
                ...
                if (isGlassDome)
                {
                    ...
                    if (animHeight > 0)
                    {
                        ...
                        //dropsUV.y = (animHeight + _Time.y) / 10.0;
                        float2 dropsTwo = SampleDropsTex2DLOD(dropsUV.xy); //r11.xy
